...
- **사용자 확인 대기**: 없음. 사용자가 README 검토·갱신을 승인해 "개발 기간과 역할"의 AI 코딩 도구(Codex CLI, Claude Code) 역할 분담을 사실대로 정리했다. 이번 정리에서 다듬은 문구 하나하나를 사용자가 별도로 확인한 것은 아니다.
...
- [x] README에 확인된 개발 기간, 개인·팀 여부, 본인 역할과 구현 범위, 결과와 한계를 기록한다. 기간은 커밋 이력(2026-09-06~2026-09-22)에서 확인했고, 선택 과제 4개 구현 이후 하지 않았거나 확인하지 못한 것(비인증 API 한도, 사람이 직접 본 확인 없음, Formspree 외부 서비스 의존, 제출용 스크린샷이 선택 과제 이전 시점)을 한계로 적었다. 사용자 승인으로 "개발 기간과 역할"에 Codex CLI·Claude Code의 역할 분담도 사실대로 정리했다(2026-09-24).
- [x] README의 관련 문서와 이미지는 상대경로로 연결한다. 022~025 학습 기록 링크를 포함해 상대경로만 쓰는 것을 확인했다(2026-09-24).
