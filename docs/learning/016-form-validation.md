...
요구사항의 "필수값과 이메일 형식을 검증하고 오류를 해당 필드 근처에 표시한다", "제출 시 기본 동작을 막는다"에 해당한다. 검증을 통과했을 때의 **성공 안내는 다음 단계**인 [017](017-form-success-state.md)에서 이어 구현했고, 실제 전송은 선택 과제로 남아 있다.
...
위 두 리스너에는 성공 안내를 지우는 `renderFormSuccess('')` 호출이 함께 들어 있다. 검증과는 다른 상태를 다루므로 [017](017-form-success-state.md)에서 설명한다.
...
`preventDefault()`는 그대로 호출되므로 페이지는 이동하지 않는다. 세 필드 모두 빈 문자열을 돌려받아 오류 표시가 지워지고, `invalidFields`가 빈 배열이라 포커스 이동도 없다. 이 단계에서는 여기서 끝났고, 지금은 이어서 성공 안내가 표시된다([017](017-form-success-state.md)).
...
| 모두 채우고 제출 | 오류 없음, 페이지 이동 없음, 입력값 유지(현재는 성공 안내도 표시) |
...
- **유효 입력 제출.** 세 필드 모두 오류 없음(`aria-invalid="false"`, 빈 문구)이었고, 주소는 제출 전후가 같았으며(`urlUnchanged: true`), 입력값 `["지호", "jiho@example.com", "포트폴리오 문의드립니다."]`가 그대로 남아 있었다. 포커스 이동도 없었다. 이 시점에는 성공 안내를 구현하기 전이어서 화면 변화가 없는 것이 예상된 결과였다.
