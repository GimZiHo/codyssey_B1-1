...
세 장 모두 정식 Google Chrome 153.0.8010.52에서 캡처했습니다. 프로젝트 카드는 실제 GitHub API 응답을 그대로 사용했습니다. **촬영 시점은 선택 과제(언어별 필터링, 시스템 테마 감지, Hero 타이핑 효과, Formspree 문의 폼 전송) 구현 이전**이라 화면에는 선택 과제 기능이 보이지 않습니다.
...
| `Array.prototype.filter` | 언어별 프로젝트 필터링 | 원본 `repositories` 배열은 그대로 두고, 선택한 언어로 거른 새 배열만 그립니다 |
| `matchMedia('(prefers-color-scheme: dark)')` | 시스템 테마 감지 | 저장된 사용자 선택이 없을 때만 OS 설정을 초기값과 `change` 이벤트에 반영합니다 |
| `setInterval` | Hero 타이핑 효과 | 100ms마다 글자 수를 늘려 타이핑처럼 보이게 하고, `prefers-reduced-motion: reduce`에서는 타이머 없이 바로 완성 문장을 표시합니다 |
| Formspree + `fetch` POST | 문의 폼 실제 전송 | 서버 구축 없이 폼 데이터를 외부 엔드포인트로 보내고, 응답에 따라 `idle`/`submitting`/`success`/`error` 상태를 관리합니다 |
...
- `src/js/main.js`: 메뉴 토글, 스크롤에 따른 버튼·배경 전환, 테마 저장·복원과 시스템 테마 감지, 섹션 등장 관찰, 문의 폼 검증과 Formspree 전송, GitHub API 요청·언어별 필터링·상태별 렌더링, Hero 타이핑 효과를 연결합니다.
- `src/images/`: 프로필 사진(`profile.jpg`, 936×1247)을 둡니다.
- `src/tests/`: 브라우저 자동 검증 스크립트와 실행 결과입니다. 버전 관리에는 넣지 않으므로(`.gitignore`) 저장소를 clone하면 곧바로 실행할 수 없고, 검증 근거는 [학습 기록](docs/learning/README.md)에 남깁니다.
...
3. **스크롤**: 60px 이상이면 내비게이션 배경이 바뀌고, 300px 이상이면 오른쪽 아래에 맨 위로 버튼이 나타납니다.
...
7. **폼 검증과 실제 전송**(전송은 선택 과제): 빈 값으로 제출하면 필드마다 오류가 표시되고, 이메일 형식이 맞지 않아도 오류가 나옵니다. 검증을 통과하면 버튼이 잠기고 Formspree로 실제 전송하며, 서버가 성공으로 응답할 때만 성공 안내가 뜨고 폼이 초기화됩니다. 서버·네트워크가 실패하면 오류 안내가 뜨고 폼은 그대로 남습니다.
8. **언어별 필터링**(선택 과제): Projects 영역의 언어 선택지를 바꾸면 원본 저장소 목록은 그대로 둔 채 해당 언어의 카드만 다시 그려집니다.
9. **시스템 테마 감지**(선택 과제): 저장된 테마 선택이 없는 상태에서 OS의 다크·라이트 설정을 바꾸면 화면 테마도 함께 바뀝니다. 한 번이라도 테마 버튼을 눌렀다면 그 뒤로는 저장된 값이 우선합니다.
10. **Hero 타이핑 효과**(선택 과제): 페이지를 열면 Hero 문구가 한 글자씩 나타납니다. OS의 "동작 줄이기" 설정을 켜면 타이핑 없이 문장이 바로 표시됩니다.

자동 검증 스크립트는 `src/tests/`에 있지만 버전 관리에 포함하지 않으므로(`.gitignore`), 저장소를 clone한 직후에는 실행할 수 없습니다. `-s`로 시나리오를, `-v`로 화면 크기를 골라 실행할 수 있고, `--list`로 목록을 봅니다. 실행에 필요한 도구 경로는 [도구 환경](docs/tool-environment.md)에 정리했습니다.
...
**필수 기능 결과(선택 과제 구현 이전)**: 정식 Google Chrome 153.0.8010.52에서 시나리오 11개(자산 로드, 반응형, 메뉴, 앵커 이동, 내비 배경, 맨 위로, 테마, 등장 효과, 소개 영역, GitHub API, 문의 폼)를 375·768·1280px에서 실행해 **462건 모두 통과**했습니다. 실제 `api.github.com` 호출도 HTTP 200으로 저장소 3건을 받아 카드로 그려지는 것을 확인했습니다. 과정과 판단 근거는 [기능 통합 검증](docs/learning/019-integration-verification.md)에 있습니다.

배포한 사이트에서도 같은 브라우저로 375·768·1280px의 진입점 이동, 자산 로드, 반응형, 메뉴, 스크롤 경계(60px·300px), 실제 GitHub API 호출, 등장 효과, 폼 검증, 테마 저장·복원을 확인해 **64건 모두 통과**했습니다(같은 시점, 선택 과제 구현 이전).

```bash
node src/tests/deployed-site-check.cjs
```

**선택 과제 결과**: 전체 재실행 대신 기능별로 따로 검증한 기존 기록을 그대로 인용합니다. 언어별 필터링은 정식 Google Chrome 153.0.8010.52로 375·768·1280px 105건과 기존 회귀 210건([022](docs/learning/022-array-filter-language.md)), 시스템 테마 감지는 Playwright Chromium headless shell로 6건([023](docs/learning/023-prefers-color-scheme.md)), Hero 타이핑 효과는 같은 환경에서 일반 6건·`prefers-reduced-motion` 2건([024](docs/learning/024-hero-typing.md))이 통과했습니다. 문의 폼 실제 전송은 모킹한 요청 기준 105건이 통과했고, 합성 데이터로 실제 Formspree 엔드포인트에 보낸 자동 요청 1건이 HTTP 200으로 응답했으며, 사용자가 Formspree 웹 대시보드의 Inbox에서 해당 제출의 수신을 직접 확인했습니다([025](docs/learning/025-formspree-submit.md)).
...
각 상태를 **바꾸는 지점**과 **그리는 지점**을 나눠 구현했습니다.

- **테마**: 버튼 클릭 → `html`의 `data-theme`과 `localStorage`의 `theme` 키 → CSS 변수 교체로 배경·글자·카드 그림자·안내 문구 색이 함께 바뀝니다. 저장된 유효한 값이 있으면 그 값을 우선 적용하고, 저장된 값이 없을 때만 `matchMedia('(prefers-color-scheme: dark)')`의 초기값과 이후 `change` 이벤트를 반영합니다(선택 과제). 한 번이라도 버튼으로 직접 바꾸면 그 뒤로는 저장된 값이 시스템 설정 변경을 가립니다.
- **API 요청과 언어 필터**: `projectState` 하나가 `idle`·`loading`·`success`·`error`와 선택 과제로 추가한 `selectedLanguage`를 담고, `renderProjects()`만 화면을 그립니다. 요청 중에는 재시도를 눌러도 요청이 겹치지 않습니다. 403·429는 요청 한도 안내를, 404와 연결 실패는 각각의 안내를 표시합니다. 언어 선택지를 바꾸면(선택 과제) `change` 이벤트가 `projectState.selectedLanguage`만 갱신하고, `renderProjects()`가 `filterRepositoriesByLanguage()`로 거른 배열을 다시 그립니다. 원본 `repositories`는 바뀌지 않습니다.
- **폼 검증과 전송**: 필드마다 입력·오류 표시 요소·검증 규칙을 함께 두고, `submit`에서 기본 동작을 막고 검사합니다. 오류가 표시된 필드만 `input`에 맞춰 다시 검사하며, 값을 고치면 직전 제출의 성공 안내를 지웁니다. 검증을 통과하면 `contactState.status`가 `idle → submitting → success/error`로 바뀌며 버튼을 잠그고 안내 문구를 갱신합니다(전송은 선택 과제). `fetch` POST가 성공으로 응답할 때만 폼을 초기화합니다.

API 응답의 문자열은 `innerHTML`에 넣기 전에 HTML에서 뜻을 가지는 문자를 바꾸고, 링크는 `https://github.com/`로 시작하는 주소만 만듭니다.

## 개발 기간과 역할

- **기능 개발 기간**: 2026-09-06 ~ 2026-09-22 (git 커밋 이력 기준). 필수 기능은 2026-09-19까지, 선택 과제 4개는 2026-09-21~2026-09-22에 구현했습니다.
- **이번 README 정리**: 2026-09-24. 기능 추가 없이 문서만 현재 코드와 진행 상태에 맞게 갱신했습니다.
- **형태**: 개인 학습 과제
- **역할 분담**: 사용자가 요구사항 정리, 작업 범위 지시, 결과 검토와 수용 판단을 직접 했습니다. Codex CLI가 작업 범위 결정·Claude Code 위임·소스 검토·커밋을 맡았고, Claude Code가 구현·검증·학습 기록 작성을 맡았습니다. 개념별로 무엇을 왜 그렇게 정했는지 [학습 기록](docs/learning/README.md)에 남겼습니다.
...
- **자동 검증은 스크립트로 브라우저를 조작한 결과**입니다. 사람이 직접 눈으로 보며 조작한 확인은 하지 않았습니다. Formspree 실제 전송도 자동 요청이 HTTP 200으로 응답한 것을 확인한 것이며, 사람이 직접 확인한 것은 Formspree 웹 대시보드 Inbox에서의 수신 여부입니다.
- **문의 폼 실제 전송은 Formspree라는 외부 서비스에 의존**합니다. 이 서비스가 중단되거나 요금제·정책이 바뀌면 전송이 막힐 수 있습니다.
- **다크 모드에서 입력 컨트롤은 브라우저 기본 스타일**을 따릅니다. 과제가 요구한 범위 밖이라 별도로 맞추지 않았습니다.
- **제출용 스크린샷은 선택 과제 구현 이전에 캡처**했습니다. 화면에는 언어별 필터링·시스템 테마 감지·Hero 타이핑 효과·문의 폼 실제 전송이 보이지 않습니다.
- **462건·64건 자동 검증도 선택 과제 이전 시점의 근거**이며, 선택 과제는 기능별로 별도 검증했습니다(위 "확인 방법" 참고).
...
| [022 언어별 필터링](docs/learning/022-array-filter-language.md) | `filter`로 원본 배열을 바꾸지 않고 거르기 |
| [023 시스템 테마 감지](docs/learning/023-prefers-color-scheme.md) | 저장된 선택과 `prefers-color-scheme` 우선순위 |
| [024 Hero 타이핑 효과](docs/learning/024-hero-typing.md) | `setInterval` 타이핑과 `prefers-reduced-motion` 대응 |
| [025 Formspree 문의 폼 전송](docs/learning/025-formspree-submit.md) | 실제 엔드포인트로 `fetch` POST와 상태 관리 |
